...
let check_dups args f =
  (List.fold args ~init:IdSet.empty ~f:(fun acc ->
       function
  ...
  | Pid key, data ->
    ...
    then (
      ...
      | Some t ->
        ...
        then { ctx with locals = Map.remove ctx.locals key }
    ...
    (match Option.map ~f:(normalize_type ctx) (Map.find ctx.locals x) with
     | Some Tbool ->
       ...
       then Errors.modified_condition x
         ...
         let diff =
           Map.symmetric_diff ~data_equal:(type_norm_equiv ctx) ctx1.locals ctx2.locals
         in
...
let check_decls ?(init = Context.empty) (decls : Ast.decl list) =
  List.fold decls ~init ~f:(fun ctx decl ->
    ...
    | Dstatic a ->
      ...
      let statics =
        match Map.add ctx.statics ~key:a.name ~data:(Tptr (Some a.elt_type)) with
        | `Ok m -> m
