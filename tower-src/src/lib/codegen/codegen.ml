  ...
  @@ List.mapi (List.sub ~pos:0 ~len:k ws2) ~f:(fun i w ->
    ...
    then
      ...
      @@ List.init (1 lsl i) ~f:(fun n ->
        ...
        Gates.list
          [ Pnot (w', [ w; List.nth_exn ws1 n ]); Pnot (List.nth_exn ws1 n, [ w' ]) ]))
;;

let prop_left ws1 =
  Gates.concat
  @@ List.mapi ws1 ~f:(fun i w ->
    if i = 0 then Gates.list [] else Gates.list [ Pnot (w, [ List.nth_exn ws1 (i - 1) ]) ])
;;
...
let binary_op a bop ws1 ws2 ws =
  ...
  | Beq ->
    let w =
      ...
      | _ -> assert false
    in
    let anc = Alloc.alloc_n a (List.length ws1) in
    let g =
      Gates.concat
      @@ List.map ~f:Gates.list
      @@ List.map3_exn
           ~f:(fun w1 w2 w ->
             [ Pnot (w, [ w1; w2 ])
             ; Pnot (w1, [])
           ...
           anc
    in
    with_uncomputed g @@ Gates.list [ Pnot (w, anc) ]
  | Bless ->
    let w =
      match ws with
      | [ w ] -> w
