...
let rec show_stmt : stmt -> string list = function
  ...
  | Swith (s1, s2) ->
    [ "with {" ]
    @ indent (List.concat_map ~f:show_stmt s1)
    @ [ "} do {" ]
    @ indent (List.concat_map ~f:show_stmt s2)
    @ [ "}" ]
;;

let show_modul (m : modul) =
  let args =
    "("
    ^ String.concat
        ~sep:", "
...
let show_moduls (ms : modul list) : string =
  List.map ~f:show_modul ms |> String.concat ~sep:"\n"
;;

let rec length = function
  | Sif (_, s) -> 1 + List.sum (module Int) ~f:length s
  | Swith (s1, s2) ->
    List.sum (module Int) ~f:length s1 + List.sum (module Int) ~f:length s2
